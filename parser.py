...
for month in range(1995,2020):
    url_string = "https://www.fangraphs.com/leaders.aspx?pos=all&stats=pit&lg=all&qual=0&type=3&season="
    url_month  = str(month)
...
for month2 in range(1995,2020):
    ...
    if(month2>1997):
        ...
        for i in range(0,30):
            for j in range(1,16):
                if(j==1): FILE_ans.write('{:15}'.format('"'+ans[i*17+j]+'"'))
                else: FILE_ans.write('{:13}'.format('"'+ans[i*17+j]+'"'))
            FILE_ans.write("\n")
